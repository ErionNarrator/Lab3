...
app.Map("/login", async (User user, DataContext db) =>
{
List<ReportÑard> list = db.ReportÑards.ToList();
User? person = await db.Users!.FirstOrDefaultAsync(p => p.Login == user.Login && p.Password == user.Password);
